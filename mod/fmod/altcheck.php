			...
			<div class="widescroll-bgimg">
				...
				<div class="widescroll-content">
                                        <?php
                                            ...
                                            elseif(isset($_GET['username']) && $user->doesExist($_GET['username']))
                                            {
                                                ?>
                                                    ...
                                                    <table cellspacing="6" style="border:1x dotted white;">
                                                        <?php
                                                            foreach(queryAlts($_GET['username'], $user->getUserIp($_GET['username']), $user->getLastIp($_GET['username']), $database) as $user)
                                                            {
                                                                ?>
                                            ...
                                            {
                                                //search
                                                $search = $database->processQuery("SELECT `username`,`ip`,`lastip` FROM `users` WHERE `username` LIKE ? ORDER BY `username` ASC", array('%'.$_POST['username'].'%'), true);
                                                ...
                                                if($database->getRowCount() >= 1)
                                                {
                                                    ...
                                                    foreach($search as $result)
                                                    {
                                                        ...
                                                        queryAlts($result['username'], $result['ip'], $result['lastip'], $database);
                                            ...
                                            function queryAlts($username, $ip, $lastip, database $database)
                                            {
                                                return $database->processQuery("SELECT `username` FROM `users` WHERE (`ip` = ? OR `lastip` = ?) AND `username` <> ? ORDER BY `username` ASC", array($ip, $lastip, $username), true);
                                            }
                                        ?>
				</div>
			</div>
