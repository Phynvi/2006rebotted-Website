			...
			<div class="widescroll-bgimg">
				<div class="widescroll-content">
                                        <?php
                                            ...
                                            elseif(isset($_GET['toggle_donator']))
                                            {
                                                //toggle their donator status
                                                $isDonator = $user->isDonator($s_user);
                                                $database->processQuery("UPDATE `users` SET `donator` = ? WHERE `username` = ? LIMIT 1", array(($isDonator) ? 0 : 1, $s_user), false);
                                                
                                                echo 'You have successfully '. (($isDonator) ? 'revoked' : 'added') .' '. $s_user .'\'s donator status. <a href="?username='. $s_user .'">Back</a>';
                                            }
                                            else
                                            {
                                                //let's extract the user's information
                                                $info = $database->processQuery("SELECT `id`,`reg_date`,`acc_status`,`ip`,`lastlogin`,`lastpost`,`lastip`,`donator` FROM `users` WHERE `username` = ?", array($s_user), true);
                                                ...
                                                ?>
                                                    ...
                                                    <table>
                                                        <tr>
                                                            <td>Username</td>
                                                            <td><?php echo $user->dName($s_user, $info[0]['acc_status']); ?></td>
                                                            <td><a href="?username=<?php echo $s_user; ?>&change_rank=1">Promote/Demote</a></td>
                                                            <td><a href="?toggle_donator=1&username=<?php echo $s_user; ?>"><?php echo ($info[0]['donator'] == 1) ? 'Revoke Donator' : 'Give Donator'; ?></a></td>
                                                        </tr>
                                                        <tr><td>Profile</td><td><a href="../forums/profile.php?username=<?php echo $s_user; ?>" target="_blank">Click here to view profile.</a></td></tr>
