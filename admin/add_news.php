...
else
{
    ...
    else
    {
        ...
        if($_POST['f_post'] != 'false' && in_array($_POST['f_post'], $forums)) 
                $database->processQuery("INSERT INTO `threads` VALUES (null, ?, ?, ?, ?, NOW(), ?, NOW(), ?, '', ?, ?, '0', '0', '0', '', '0', ?, '0', '0', '')", array($_POST['f_post'], $_POST['title'], $content, $username, qfc($database), $username, time(), $_SERVER['REMOTE_ADDR'], time()), false);
