...
function belowTitleOptions($username_specific, $rankOfViewer)
{
    ...
    if($rankOfViewer > 2)
    {
        $belowTitle = '<br/><br/><a href="../mod/fmod/infract.php?target='. $username_specific .'">Infract</a><br/><a href="../mod/fmod/mute.php?target='. $username_specific .'">Mute/Unmute</a>';
	...
	<body>
		...
		<div id="infopane">
                        <?php
                            if($rank > 3)
                            {
                                $database->processQuery("SELECT * FROM `threads` WHERE `escalated` = 1", array(), false);
                                if($database->getRowCount() > 0) echo '<span style="color:red">There are '. $database->getRowCount() .' thread(s) that need an administrator\'s attention.<br/>Go <a href="../admin/viewescalated.php">here</a> to view the thread(s).</span><br/><br/>';
                            }
                        ?>
                ...
                <?php
                    if($rank > 2)
                    {
                        ?>
                            ...
                            <div id="mod_controls" name="extra" style="display:none;margin-bottom:4px;" id="extra">
                                Please type in the reason for escalating the thread.<br/><br/>
                                <input type="text" name="reason" maxlength="75"><input type="submit" id="escalate_submit" value="Escalate">
                            </div>
                
                            <div id="mod_controls">
                                <?php if(!$thread->isEscalated($i) && $rank == 3) { ?><a href="#" data="<?php echo $i; ?>" id="escalate">Escalate</a>&nbsp;&nbsp; <?php } ?>
                                                ...
                                                <tbody>
                                                                        <tr>
                                                                                ...
                                                                                <td class="rightpanel">
                                                                                        ...
                                                                                        <div class="msgcontents">
                                                                                                <?php echo $base->br2nl((empty($details['moved'])) ? $thread->formatPost($details['content'], $details['username'], $base, $user, $forum) : $details['content']); ?>
                                                                                                </div>
                                                    ...
                                                    <tbody>
                                                                            <tr>
                                                                                    ...
                                                                                    <td class="rightpanel">
                                                                                            <div class="msgtime"><?php echo $date; ?></div>
                                                                                            <div class="msgcontents">
                                                                                                    <?php echo $base->br2nl($thread->formatPost($reply['content'], $reply['username'], $base, $user)); ?>
                                                                                                    </div>
		...
		<div id="breadcrumb">
         ...
         <?php if ($rank == 3) echo '<script type="text/javascript" src="../js/escalate.js"></script>'; ?>
