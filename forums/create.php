...
if(isset($_POST['message']) && isset($_POST['title']))
{
    ...
    else
    {
        ...
        //insert thread
        $database->processQuery("INSERT INTO `threads` VALUES (null, ?, ?, ?, ?, NOW(), ?, NOW(), ?, '', ?, ?, '0', '0', '0', '0', ?, ?, '0', '0', '')", array($f, $title, $message, $username, qfc($database), $username, time(), $_SERVER['REMOTE_ADDR'], $hide, time()), false);
...
else
{
    ...
    <tr>
        ...
        <td class="commandinput"><input style="margin-left:10px; float:left;" size="40" maxlength="'. $title_length .'" id="charlimit_text_b" type="text" class="textinput" name="title"/></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <l1><textarea id="charlimit_text_a" name="message" listener="1" rows="20" cols="60"></textarea></l1><br />
...
<?php
    if($rank > 3){
        ?>
            ...
            <script type="text/javascript" src="../js/toggleeditor.js"></script>
        <?php
    }
?>
