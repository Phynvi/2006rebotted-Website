                        ...
                        <div id="t_container_bottom">
                                <?php
                                    if($rank < 4)
                                        ...
                                        $read = $database->processQuery("SELECT `id`,`title`,`date` FROM `messages` WHERE (`opened` = 1 AND `receiver` = ?) OR (`lastreply` = ? AND `receiver` = '!') OR (`receiver` = '!' AND `status` = 1) AND ". time() ." - `timestamp` < '7889231' ORDER BY `date` DESC", array($username, $username), true);    
