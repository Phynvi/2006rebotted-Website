...
if($user->isLoggedIn())
{
	...
	if($rank > 1)
	{
		?>
			...
			<li class="i-pmod"><a href="forums/viewforum.php?forum=<?php echo $config['reportforum']; ?>">View reports</a></li>
			</ul>
			</fieldset>
	...
	?>
		...
		<li class="i-appeal"><a href="offences.php">View Offenses</a></li>
		</ul>
		</fieldset>
	<?php
}
...
//withdraw custom page categories
$categories = $database->processQuery("SELECT `id`,`cat_title` FROM `page_categories` ORDER BY `id` ASC", array(), true);
...
foreach($categories as $category)
{
    ...
    <fieldset class="menu web">
    <legend><?php echo $category['cat_title']; ?></legend>
    <ul>
    
    <?php
        $pages = $database->processQuery("SELECT `id`,`page_title`,`access_level` FROM `pages` WHERE `parent` = ? ORDER BY `id` ASC", array($category['id']), true);
    
        foreach($pages as $page)
        {
            if($rank >= $page['access_level']) { ?> <li class="i-files"><a href="page.php?id=<?php echo $page['id']; ?>"><?php echo $page['page_title']; ?></a></li> <?php }
        }
    ?>
    </ul>
    </fieldset>
